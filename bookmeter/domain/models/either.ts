 ...
 * @param f 適用する関数
 */
export const chain = <E, A, B>(f: (a: A) => Either<E, B>) => (fa: Either<E, A>): Either<E, B> => {
  if (isRight(fa)) {
    return f(fa.right);
  }
  return fa as unknown as Either<E, B>;
 ...
 */
export const fold = <E, A, B>(onLeft: (e: E) => B, onRight: (a: A) => B) => (fa: Either<E, A>): B => {
  if (isRight(fa)) {
...
export const tryCatchAsync = <E, A>(
  ...
  return promise
    ...
    .catch(error => left<E, A>(onError(error)));
};
 ...
 */
export const getOrElse = <E, A>(defaultValue: A) => (fa: Either<E, A>): A => {
  if (isRight(fa)) {
    return fa.right;
  }
  return defaultValue;
 ...
 */
export const filterOrElse = <E, A>(predicate: (a: A) => boolean, onFalse: (a: A) => E) => 
