...
export class IsbnService {
  ...
  static convertISBN10ToISBN13(isbn10: string): Result<ISBN13> {
    ...
    const result = pipe(
      ...
      toEither,
      // 検証結果を確認
      chain((isValid) => (isValid ? right(undefined) : left(new Error("有効なISBN-10ではありません")))),
      // 変換処理
      chain(() =>
        tryCatch<Error, ISBN13>(
          () => {
          ...
          (error): Error =>
            error instanceof Error ? error : new Error("ISBN-10からISBN-13への変換中にエラーが発生しました")
        )
      )
    );
