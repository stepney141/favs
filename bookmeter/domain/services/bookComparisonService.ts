...
export class BookComparisonService {
  ...
  static mergeWithDiff(baseList: BookList, diff: BookListDiff): BookList {
    // 削除された書籍を除去
    let resultList = diff.removed.reduce((list, book) => list.remove(book.isbn.toString()), baseList);

    // 追加された書籍を追加
    resultList = diff.added.reduce((list, book) => list.add(book), resultList);

    // 変更された書籍を更新
    resultList = diff.changed.reduce((list, change) => {
